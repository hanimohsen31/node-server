...
async function loginAndNavigateToDashboard(page, baseUrl, dashboardId) {
  ...
  const dashboardResPromise = page.waitForResponse(
    (resp) => {
      const urlMatches = resp.url().includes(dashboardApiUrl)
      const goodStatus = [200, 304].includes(resp.status())
      return (urlMatches && goodStatus, { timeout: 1_000_000 })
    },
    { timeout: 1_000_000 }
  )
  await page.goto(`${baseUrl}/dashboard/${dashboardId}`, {
...
router.route('/api/dashboards/export/dashboard/:id').post(GetDashboardScreenShot)
