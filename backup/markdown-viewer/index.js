...
const app = express()
const ROOT_DIR = path.join(__dirname, 'markdown')
const files = fs
...
async function GetMarkdownList(req, res) {
  let list = files.map((file) => `<li><a href="/view/${encodeURIComponent(file)}">${file}</a></li>`).join('')
  list += '<li><a href="/view/overall">Overall (Live)</a></li>'
  let htmlContent = `
...
// ----------------  DIVIDER  routes -----------------------------------
app.get('/', GetMarkdownList)
app.get('/view/:file', GetMarkdownFile)
app.post('/create-overall', CreateOverall)

// ----------------  DIVIDER  server -----------------------------------
app.listen(3000, () => {
  const url = 'http://localhost:3000'
  console.log(`Markdown viewer running at ${url}`)
